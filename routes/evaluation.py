...
def view(cid):
    if request.method == 'GET':
        ...
        if not course_data:
            flash('查無課程資料', 'error')
            error = True
            return render_template('view_evaluation.html', nodata=1, button_disable=1, cid='', evaluation_data='',
                                   avg_sweetness='', avg_cool='', avg_gain='',
                                   serial_no='----', term='',
                                   year='', department='', course_name='', course_code='',
                                   restrict='', quota='', authorize_quota='',
                                   interschool_quota='', instructor='', reg_sel='',
                                   credits='', english='', time='', location='', note='')
        else:
            ...
            return render_template('view_evaluation.html', nodata=0, button_disable=0, cid=cid, evaluation_data=evaluation_data,
...
def add(user, cid):
    if request.method == 'GET':
        checkcourse = Course.query.filter_by(cid=cid).first()
        if not checkcourse:
            flash('無法新增評價，因為:', 'error')
            error = True
